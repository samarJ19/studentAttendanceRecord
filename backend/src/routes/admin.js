  ...
  include: {
    ...
    teachingAssignments: {
      include: {
        teacher: {
          include: {
            user: true, // ✅ include the nested user here
          },
        },
      },
    },
...
router.post("/teaching-assignments", async (req, res) => {
  const { teacherId, courseId, branchId, semester, academicYear, section } =
    req.body;
  ...
  try {
    ...
    const assignment = await prisma.teachingAssignment.create({
      ...
      include: {
        //we are including the teacher which is being assigned to the course
        teacher: {
          include: {
            user: true,
          },
